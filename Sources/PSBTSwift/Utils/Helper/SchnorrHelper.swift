...
public struct SchnorrHelper{
    ...
    public static func tweakedOutputKey(publicKey: Data, compressed: Bool = true) throws -> Data {
        ...
        var outputKey = try SchnorrHelper.addEllipticCurvePoints(a: internalKey, b: tweakedPublicKey)
        var pubKeyLen = 33
        var outputKeyBytes = [UInt8](repeating: 0, count: pubKeyLen)

        guard secp256k1_ec_pubkey_serialize(SchnorrHelper.context, &outputKeyBytes, &pubKeyLen, &outputKey, UInt32(compressed ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED)) == 1 else {
            throw SchnorrError.keyTweakError
        }
    ...
    public static func sign(data: Data, privateKey: Data, publicKey: Data) throws -> Data {
        ...
        guard secp256k1_keypair_create(SchnorrHelper.context, &keypair, tweakedPrivateKey.bytes) == 1,
